...
#include "PTAMTracking/ShiTomasi.h"
#include "PTAMTracking/SmallMatrixOpts.h"
#include "PTAMTracking/PatchFinder.h"
...
bool Tracker::HandleKeyPress( string sKey )
{
	...
	else if (sKey == "v")//yasuahra added by yasuhara 2010/10/27
	{
		// change mapview mode
		GUI.ParseLine("mapview");
	} else if (sKey == "s")//added by yasuhara 2010/10/27
	{
		GUI.ParseLine("screenshot");
	} else if (sKey == "BackSpace")//added by Umakatsu 2011.1.27
	{
		GUI.ParseLine("retrieve");
	} else if (sKey == "PageUp")//added by Umakatsu 2011.1.27
	{
		GUI.ParseLine("next");
	} else if (sKey == "PageDown")//added by Umakatsu 2011.1.27
	{
		GUI.ParseLine("back");
	}
  return false;
...
/**
 * Time to add a new keyframe? The PTAMTracking/MapMaker.handles most of this.
 */
