  ...
  while(!mbDone)
	...
	*mgvnOptimizing = 0;
      ...
      else
	{
      ...
      const int image_width = imFrameRGB.size().x;
      const int image_height = imFrameRGB.size().y;
      IplImage * dst_image;
      // dst_image = cvCreateImage(cvSize(image_width,image_height) , IPL_DEPTH_8U ,3);
  
      int datasize2 =  image_width * image_height * 3;  // ppm(カラー)用
      // int line , row;
      unsigned char * dst = new unsigned char[datasize2];
      // for(int i = 0; i < datasize2; i+=3){  // カラー画像
      // 	line = div(i/3, image_width).quot;
      // 	row = div(i/3, image_width).rem;
      // 	dst[i] = imFrameRGB[line][row].red;
      // 	dst[i + 1] = imFrameRGB[line][row].green;
      // 	dst[i + 2] =imFrameRGB[line][row].blue;
      // }
      // int channel = 3;
      // for(int y=0; y<image_height; y++) for(int x=0; x<image_width; x++){
      // 	  dst_image->imageData[ ( y * dst_image->widthStep) + (x * channel) + 2] = dst[ ( x + y * image_width) * channel + 0];	//R
      // 	  dst_image->imageData[ ( y * dst_image->widthStep) + (x * channel) + 1] = dst[ ( x + y * image_width) * channel + 1];	//G
      // 	  dst_image->imageData[ ( y * dst_image->widthStep) + (x * channel) + 0] = dst[ ( x + y * image_width) * channel + 2];	//B
      // }  
      myReadRawimage(imFrameRGB, dst);
      dst_image = mycvConvertIplImage(dst, image_width, image_height, 3);
      cvShowImage ("Capture", dst_image);
      cvWaitKey(3);
